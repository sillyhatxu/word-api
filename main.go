...
const (
	dataSourceName = `sillyhat:sillyhat@tcp(localhost:3306)/sillyhat`
	maxIdleConns   = 10
	maxOpenConns   = 20
)
...
func main() {
	...
	wordJson, _ := ioutil.ReadFile("words_dictionary.json")
	var wordMap map[string]int
	err := json.Unmarshal([]byte(string(wordJson)), &wordMap)
	if err != nil {
		log.Fatal(err)
	}
